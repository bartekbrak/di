...
function query($head, $db, $raw = false) {
    ...
    $query = sprintf("SELECT id,head,definition FROM %s WHERE head='%s'", mysql_real_escape_string($db), mysql_real_escape_string($head));
    $result = mysql_query($query);
    if (!$result)
        die("<error>Database access failed: " . mysql_error() . "\n<br />on query:" . $query . "</error>");
...
switch ($_REQUEST['action']) {
    ...
    // mylog($_REQUEST['definition'] . "\n");
        ...
        if (isset($_REQUEST['db']) && isset($_REQUEST['head']) && isset($_REQUEST['definition'])) {
            ...
            if ($hid) {
                $query = sprintf("UPDATE %s SET head='%s', definition='%s' WHERE id='%s'", mysql_real_escape_string($db), mysql_real_escape_string($head), mysql_real_escape_string($definition), mysql_real_escape_string($hid));
                $result = mysql_query($query);
                if (!$result)
                    die("Database access failed: " . mysql_error() . "\n<br />on query:" . $query);
            } else {
                $query = sprintf("INSERT INTO %s(head,definition, id) VALUES('%s','%s', NULL)", mysql_real_escape_string($db), mysql_real_escape_string($head), mysql_real_escape_string($definition));
                $result = mysql_query($query);
                if (!$result)
                    die("Database access failed: " . mysql_error() . "\n<br />on query:" . $query);
            ...
            printf("<mesage>OK. Saved.</message>", urldecode($head), urldecode($definition));
        }
        break;
    //end of action write
